...
procedure reverse(var p: TBytes; len: Integer); overload;
procedure reverse(var p: PByteArray; len: Integer); overload;
...
begin
  Pos := Self.Position;
  if (Pos >= 0) and (Count > 0) then begin
    EndPos := Pos + Count;
    Size:= Self.Size;
    if EndPos > Size then
...
procedure reverse(var p: PByteArray; len: Integer);
var
  i, j: Integer;
  temp: Byte;
begin
...
begin
  ...
  repeat
    ...
    while I < BytesRead do begin
      ...
      Inc(BufPtr, 4);
      Inc(K, 4);
      if Base64WrapOutput and (K > 75) then begin
        BufPtr[0] := #$0D;
        BufPtr[1] := #$0A;
        Inc(BufPtr, 2);
        K := 0;
  ...
  begin
    while True do begin
      ...
      if NumRead = 1 then begin
        if C in ['0'..'9','A'..'Z','a'..'z','+','/','='] then begin
  ...
  begin
    ...
    repeat
      ...
      while (Idx < IdxEnd) do begin
        if not (InBuf[Idx] in ['0'..'9','A'..'Z','a'..'z','+','/','=']) then begin
          Dec(IdxEnd);
          if Idx < IdxEnd then
